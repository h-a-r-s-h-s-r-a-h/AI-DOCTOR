...
def format_text(text):
    ...
    for line in lines:
        # Split the line into condition name and details
        condition, details = line.split(' - ', 1)

        # Split the details into parts based on semicolons
        parts = details.split(';')
        
        # Format the initial condition line
        formatted_text += f"{condition.strip()} - {parts[0].strip()}\n"

        # Combine the remaining parts into a single bullet point
        if len(parts) > 1:
            # Join the remaining parts after the first symptom part
            remaining_details = '. '.join(part.strip() for part in parts[1:] if part.strip())
            formatted_text += f"    • Medication and Remedies - {remaining_details.strip()}\n\n"

    return formatted_text.strip()

def assessment(llm, user_input):
    prompt_template_items2 = PromptTemplate(
        input_variables=["user_input"],
        template='''You are a doctor's assistant. Based on the patient's symptoms: {user_input}, provide assessment of patients health and well being in 150 to 200 Words'''
    )
    chain2 = LLMChain(llm=llm, prompt=prompt_template_items2)
    response1 = chain2.run({"user_input": user_input}) 
    
    return response1


@app.route('/doc_assit', methods=['POST'])
def doc_assit():
    llm = Ollama(base_url="http://localhost:11434", model="moondream")  # Use the specified model
    print("LLM loaded")
    ...
    if request.method == 'POST':
        ...
        response1 = assessment(llm, user_input)
        print("respo1 : ", response1)
        
        prompt_template_items2 = PromptTemplate(
            input_variables=["user_input"],
            template='''You are a doctor's assistant. Based on the patient's symptoms: {user_input}, provide a list of possible diseases. For each disease, include: - **Disease Name**: - **Description**: - **2 Diagnostic Tests**: - **3 Medicines**: - **Precautions**: in bullets in more than 500 words and not more than 4 points '''
        )
        chain2 = LLMChain(llm=llm, prompt=prompt_template_items2)
        response2 = chain2.run({"user_input": user_input})  # Limit max tokens for speed
        response2 = format_text(response2) 
        response2 = response2.replace('*', '').strip()
        
        combined_text = f"""Assessment:
{response1}

Potential Diagnoses:
{response2}"""

        history.update({"data": combined_text})
        
        print("Possible Diseases and Details:", combined_text)
